...
from ret.space.space import ContinuousSpaceWithTerrainAndCulture3d, Precipitation
from ret.testing.mocks import MockModel3d

if TYPE_CHECKING:
    from typing import Any, Optional

    from ret.sensing.sensor import JohnsonCriteria, PerformanceCurve
...
class TestAcquireSensor(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ...
            [1000, 0.75, 2.0, 0.0, 0.553088],  # Vary meteorological vis. low
            [1000, 0.75, 5.0, 0.0, 0.703323],  # Vary meteorological vis. mid
            [1000, 0.75, 9.0, 0.0, 0.795377],  # Vary meteorological vis. high
    ...
    def test_ti_get_observation_confidence_probabilities(self):
        """Test ti sensor at 0 attenuated distances."""
        assert self.ti_sensor._get_observation_confidence_probabilities(
            1000, 0, self.target_agent, timedelta(minutes=5)
        ) == (1.0, 1.0, 1.0)

    def test_ti_sensor_warning(self):
        """Test ti sensor will give warning for > 1 relative_humidity."""
        relative_humidity = 5

        with self.assertRaises(ValueError) as e:
            self.ti_sensor._get_atmospheric_transmission(1000, relative_humidity, 320.0, 0.0)
        self.assertEqual(
    ...
    def test_acquire_1d_sensor_magnification_error(self, magnification):
        ...
        assert f"Acquire 1D Sensor magnification value ({magnification}) must be positive." == str(
    ...
    def test_no_line_of_sight(self):
        """Test that the 1d acquire sensor works without line of sight."""
