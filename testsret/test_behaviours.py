...
from ret.agents.agent import RetAgent, WeaponFiredStatus
...
from ret.behaviours.fire import FireBehaviour, HostileTargetResolver, TargetSelector
...
from ret.space.culture import Culture, default_culture
...
from ret.types import Coordinate2dOr3d  # noqa: TC001
...
class TestBehaviours(unittest.TestCase):
    ...
    def test_ground_based_move_behaviour_get_gradient_modifier(self):
        """Test calculation of gradient-based ground move modifiers."""
        behaviour = GroundBasedMoveBehaviour(
            self.base_speed, self.gradient_speed_modifiers, self.culture_speed_modifiers
        )

        assert behaviour.get_gradient_modifier(-10) == 0.8
        assert behaviour.get_gradient_modifier(0) == 1
        assert behaviour.get_gradient_modifier(11) == 0.8

    def test_ground_based_move_behaviour_get_culture_modifier(self):
...
class TestCommunicateBehaviour(unittest.TestCase):
    ...
    def test_communicate_perceived_world(self):
        """Test communication of perceived world-view."""
    ...
    def test_communicate_mission_message(self):
        """Test communication of mission message."""
...
class TestGroundBasedMoveWithFeatures(unittest.TestCase):
    ...
    def test_ground_based_move_behaviour_get_area_modifier(self):
        """Test ground-based movement get area modifier method."""
        behaviour = GroundBasedMoveBehaviour(
            self.base_speed,
            self.gradient_speed_modifiers,
            area_speed_modifiers={"area": self.area_modifier},
        )
        assert behaviour.get_area_modifier(self.agent_inside) == self.area_modifier
        assert behaviour.get_area_modifier(self.agent_outside) == 1

        behaviour_2 = GroundBasedMoveBehaviour(
            self.base_speed,
            self.gradient_speed_modifiers,
            area_speed_modifiers={"###": self.area_modifier},
        )
        with self.assertRaises(KeyError) as e:
    ...
    def test_ground_based_move_behaviour_with_area(self):
        ...
        assert (
            ...
            == self.base_speed * 1 * self.area_modifier * self.model.time_step.seconds
        )
