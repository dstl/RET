...
from ret.agents.agent import AgentSensedStatus, RetAgent
...
class TestSensor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.target_agent_22 = ProjectileAgent(
            model=self.model,
            pos=(300.0, 1.0, 22.0),
            name="Target Agent",
            base_speed=100,
    ...
    def test_distance_attenuated_sensor_no_view_angle(self):
        """Test distance attenuated sensor gives correct results."""
        distance_attenuated_sensor = DistanceAttenuatedSensor(
            distance_thresholds=SensorDistanceThresholds(400, 300, 200),
            view_angle=None,
        ...
        distance_attenuated_sensor.run_detection(sensor_agent=self.sensor_agent)
    ...
    def test_distance_attenuated_sensor_limited_view_angle(self):
        """Test distance attenuated sensor gives correct results."""
        distance_attenuated_sensor = DistanceAttenuatedSensor(
            distance_thresholds=SensorDistanceThresholds(400, 300, 200),
            view_angle=35,
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            perceived_agents = distance_attenuated_sensor.get_results(
            ...
            assert (
                len(
                    [agent for agent in perceived_agents if agent.confidence == Confidence.IDENTIFY]
                )
                == 3
            )
            ...
            assert (
                len(
                    [
                        ...
                        if agent.confidence == Confidence.RECOGNISE
                    ]
                )
                == 1
            )
            ...
            assert (
                len([agent for agent in perceived_agents if agent.confidence == Confidence.DETECT])
                == 2
            )
        assert len(w) == 1

    def test_weapon_platform_sensor_identify(self):
        """Test weapon platform sensor gives correct results."""
        distance_attenuated_sensor = WeaponPlatformLocatingSensor(
            distance_thresholds=SensorDistanceThresholds(400, 300, 200),
        )
        ...
        assert (
            len([agent for agent in perceived_agents if agent.confidence == Confidence.DETECT]) == 0
        )

    def test_weapon_platform_sensor_recognised(self):
        """Test weapon platform sensor gives correct results."""
        distance_attenuated_sensor = WeaponPlatformLocatingSensor(
            distance_thresholds=SensorDistanceThresholds(400, 300, 200),
        )

        original_pos = self.target_agent_22.pos

        self.target_agent_22.pos = (300.0, 1.0, 290)
        ...
        self.target_agent_22.pos = original_pos
    ...
    def test_sensor_look_over_terrain(
        ...
        sensor_agent = RetAgent(
            model=model,
            pos=(49.0, 75.0, 41.0),  # place at terrain height (41.0)
            name="Sensor Agent",
            affiliation=Affiliation.FRIENDLY,
            critical_dimension=2.0,
            reflectivity=0.1,
        ...
        RetAgent(
            model=model,
            pos=(75.0, 75.0, 100.0),  # place at terrain height (100.0)
            name="Target Agent",
            affiliation=Affiliation.FRIENDLY,
            agent_type=AgentType.GENERIC,
            critical_dimension=2.0,
