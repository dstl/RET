...
from ret.testing.mocks import MockModel2d, MockOrder
...
def test_model_start():
    ...
    with TemporaryDirectory() as td:
        kwargs = {"output_path": td}
        model = MockModel2d(**kwargs)
...
def test_model_finish():
    ...
    with TemporaryDirectory() as td:
        output_path = os.path.join(td, add_datetime_stamp(td))
        ...
        with open(pathlib.Path(kwargs["output_path"], "playback.json"), "r") as json_file:
        ...
        with open(
            pathlib.Path(os.path.join(str(td), get_latest_subfolder(str(td))), "playback.json"), "r"
        ) as json_file:
            json_output_results = json_file.read()
