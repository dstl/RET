...
from ret.testing.mocks import MockModel2d, MockShotFiredLogger
from ret.weapons.weapon import (
    BasicLongRangedWeapon,
    BasicShortRangedWeapon,
    BasicWeapon,
    TrackedProjectileLongRangeWeapon,
    TrackedGuidedProjectileLongRangeWeapon,
)
...
def test_tracked_weapon_firing_accuracy():
    """Test whether zero inaccuracy shots from the tracked weapon land in original location."""
    weapon = TrackedProjectileLongRangeWeapon(
        name="Long range weapon",
        radius=1.0,
        time_between_rounds=timedelta(seconds=1),
        time_before_first_shot=timedelta(seconds=0),
        kill_probability_per_round=1,
        projectile_speed_across_ground=10000,
    )
...
def test_guided_tracked_weapon_firing_accuracy():
    """Test whether shots from the guided tracked weapon land in original location."""
    weapon = TrackedGuidedProjectileLongRangeWeapon(
        name="Long range weapon",
        radius=1.0,
        time_between_rounds=timedelta(seconds=1),
        time_before_first_shot=timedelta(seconds=0),
...
def test_ignoring_fire_at_out_of_range_target_tracked():
    """Test firing at a target that is outside allowable range tracked weapon.
    ...
    weapon = TrackedProjectileLongRangeWeapon(
        ...
        min_range=3,
        max_range=20,
        projectile_speed_across_ground=1000,
    )
...
def test_ignoring_fire_at_out_of_range_target_guided():
    """Test firing at a target that is outside allowable range tracked weapon.
    ...
    weapon = TrackedGuidedProjectileLongRangeWeapon(
        ...
        min_range=3,
        max_range=20,
        projectile_speed_across_ground=10,
    )

    model = MockModel2d(time_step=timedelta(seconds=1))
    firer = RetAgent(
        name="firer",
        model=model,
        pos=(0, 0),
