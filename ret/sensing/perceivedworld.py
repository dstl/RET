...
if TYPE_CHECKING:
    ...
    from ret.types import Coordinate2dOr3d, Coordinate
...
class AgentsAt(AgentsWithinRange):
    ...
    inaccuracies in range calculations.
    """
...
class ArtilleryAgents(AgentByType):
    """Filter for artillery agents."""

    def __init__(self):
        """Create a new ArtilleryAgents filter."""
        super().__init__(agent_type=AgentType.ARTILLERY)
...
class GroupAgents(AgentByType):
    """Filter for group agents."""

    def __init__(self):
        """Create a new GroupAgents filter."""
        super().__init__(agent_type=AgentType.GROUP)


class GuidedProjectileAgents(AgentByType):
    """Filter for guided projectile agents."""

    def __init__(self):
        """Create a new GuidedProjectileAgents filter."""
        super().__init__(agent_type=AgentType.GUIDED_PROJECTILE)


class MechanisedInfantryAgents(AgentByType):
    """Filter for mechanised infantry agents."""

    def __init__(self):
        """Create a new MechanisedInfantryAgents filter."""
        super().__init__(agent_type=AgentType.MECHANISED_INFANTRY)
...
class ProtectedAssetAgents(AgentByType):
    """Filter for protected asset agents."""

    def __init__(self):
        """Create a new ProtectedAssetAgents filter."""
        super().__init__(agent_type=AgentType.PROTECTED_ASSET)


class ProjectileAgents(AgentByType):
    """Filter for projectile agents."""

    def __init__(self):
        """Create a new ProjectileAgents filter."""
        super().__init__(agent_type=AgentType.PROJECTILE)


class SensorFusionAgents(AgentByType):
    """Filter for sensor fusion agents."""

    def __init__(self):
        """Create a new SensorFusionAgents filter."""
        super().__init__(agent_type=AgentType.SENSOR_FUSION)
...
class AgentsByCasualtyState(PerceivedAgentFilter):
    ...
    def run(self, agents: list[PerceivedAgent]) -> list[PerceivedAgent]:
        ...
        Returns:
            list[PerceivedAgent]: Filtered list of agents.
        """
        return [a for a in agents if a.casualty_state == self._casualty_state]
