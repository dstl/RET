...
from ret.types import Coordinate3d, Coordinate3dBand  # noqa: TC001
...
if TYPE_CHECKING:
    ...
    from ret.types import Coordinate, Coordinate2dOr3d, Vector
...
class GroundBasedMoveBehaviour(MoveBehaviour):
    ...
    def _step(self, mover: RetAgent, destination: Coordinate) -> None:
        ...
        if distance_to_destination_2d > 0 and mover.pos is not None:
            ...
            speed = (
                self.base_speed
                * self.get_gradient_modifier(gradient)
    ...
    def get_gradient_modifier(self, gradient: float) -> float:
    ...
    def get_area_modifier(self, mover: RetAgent) -> float:
        ...
        if self.area_speed_modifiers:
            for area, area_modifier in self.area_speed_modifiers.items():
