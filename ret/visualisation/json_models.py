...
# have to be imported directly in order to support their use as types within pydantic models.
import copy
from typing import TYPE_CHECKING, Optional, Union  # noqa: TC002

from pydantic import BaseModel  # noqa: TC002

from mesa.time import TimeT  # noqa: TC002, TC001
from ret.agents.agent import RetAgent  # noqa: TC002, TC001
from ret.sensing.agentcasualtystate import AgentCasualtyState
from ret.space.space import ContinuousSpaceWithTerrainAndCulture2d  # noqa: TC002, TC001
from ret.types import Coordinate2dOr3d  # noqa: TC002, TC001
from ret.visualisation.json_icon_handler import IconCopier
from datetime import datetime  # noqa: TC002, TC001, TC003
...
class RetPlayStepData(BaseModel):
    ...
    @staticmethod
    def from_model(
        step_number: TimeT, model_time: datetime, agents: list[RetPlayAgent]
    ) -> RetPlayStepData:
        ...
        Args:
            ...
            model_time (datetime): The model time from which the data comes.
        ...
        """
        return RetPlayStepData(step_number=step_number, model_time=model_time, agents=agents)
...
class JsonOutObject(BaseModel):
    ...
    def add_timestep_data(
        self, step_number: TimeT, model_time: datetime, agents: list[RetPlayAgent]
    ):
        """Add one timestep's worth of data to the JsonOutObject.

        Args:
            step_number (TimeT): The number of the step from which the data comes.
            model_time (str): The model time from which the data comes.
            agents (list[RetPlayAgent]): A list of representations of the agents and
                their state in the recorded timestep.
        ...
        if step_number is not None:
            self.step_data.append(RetPlayStepData.from_model(step_number, model_time, agents))
        else:
            raise ValueError(
