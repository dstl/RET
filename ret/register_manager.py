...
class RetRegisterManager(Generic[T]):
    ...
    def register(self, name: str, generator: Callable[[], T]):
        ...
        """
        if name == "None":
            warnings.warn("Cannot register an item to 'None'", stacklevel=2)
            return

        if name in self._methods:
            warnings.warn(f"Overwriting the registered member: '{name}'", stacklevel=2)
    ...
    def get_register_item(self, name: str) -> Optional[T]:
        ...
        has an item registered to the "None" name.
        ...
        if name in self._methods:
            try:
                ...
                warnings.warn(
                    f"Error in registered member: '{name}'. Returning None.", stacklevel=2
                )
                return None
        warnings.warn(f"'{name}' is unregistered. Returning None.", stacklevel=2)
        return None
