...
from ret.creator.agents import create_list
...
if TYPE_CHECKING:
    ...
    from ret.agents.agent import RetAgent, Affiliation, AgentType, AgentCasualtyState
    from ret.orders.order import Trigger
    from ret.space.feature import Area, Boundary
...
class TriggerConfig:
    ...
    agent_affiliation_list: list[Affiliation] = field(default_factory=list)
    confidence_level_list: list[Confidence] = field(default_factory=list)
    agent_types_list: list[list[AgentType]] = field(default_factory=list)
    casualty_state_list: list[AgentCasualtyState] = field(default_factory=list)
    sense_range_list: list[int] = field(default_factory=list)
...
def _create_agent_sensed_trigger(
    trigger_counter: int, trigger_config_lists: TriggerConfig, trigger_list: list[Trigger]
) -> None:
    """Create an agent sensed trigger using a TriggerConfig and add to the trigger list.
    ...
    _check_arg_not_none(
        [
            trigger_config_lists.agent_affiliation_list[trigger_counter],
            trigger_config_lists.confidence_level_list[trigger_counter],
            trigger_config_lists.agent_types_list[trigger_counter],
            trigger_config_lists.casualty_state_list[trigger_counter],
        ]
    )
    optional_args = _check_optional_arg_not_none(
        trigger_config_lists.sticky_list[trigger_counter],
    ...
    trigger_list.append(
        AgentsSensedTrigger(
            agent_affiliation=trigger_config_lists.agent_affiliation_list[trigger_counter],
            confidence_level=trigger_config_lists.confidence_level_list[trigger_counter],
            agent_types=trigger_config_lists.agent_types_list[trigger_counter],
            casualty_state=trigger_config_lists.casualty_state_list[trigger_counter],
            log=optional_args[1],
            sticky=optional_args[0],
        )
    )
...
def _create_agent_sensed_in_range_trigger(
    trigger_counter: int, trigger_config_lists: TriggerConfig, trigger_list: list[Trigger]
) -> None:
    """Create an agent sensed trigger using a TriggerConfig and add to the trigger list.
    ...
    _check_arg_not_none(
        [
            trigger_config_lists.agent_affiliation_list[trigger_counter],
            trigger_config_lists.confidence_level_list[trigger_counter],
            trigger_config_lists.sense_range_list[trigger_counter],
            trigger_config_lists.agent_types_list[trigger_counter],
            trigger_config_lists.casualty_state_list[trigger_counter],
        ]
    )
    ...
    trigger_list.append(
        AgentsSensedInRangeTrigger(
            agent_affiliation=trigger_config_lists.agent_affiliation_list[trigger_counter],
            confidence_level=trigger_config_lists.confidence_level_list[trigger_counter],
            sense_range=trigger_config_lists.sense_range_list[trigger_counter],
            agent_types=trigger_config_lists.agent_types_list[trigger_counter],
            casualty_state=trigger_config_lists.casualty_state_list[trigger_counter],
            log=optional_args[1],
            sticky=optional_args[0],
...
def _create_configuration_lists(
    ...
    agent_affiliation: OptionalSingleOrList[Affiliation],
    confidence_level: OptionalSingleOrList[Confidence],
    agent_types: OptionalSingleOrList[list[AgentType]],
    casualty_state: OptionalSingleOrList[AgentCasualtyState],
    sense_range: OptionalSingleOrList[int],
) -> TriggerConfig:
    ...
    Args:
        ...
        boundary (OptionalSingleOrList[Boundary]): The boundary parameter for each trigger.
            Required by AGENT_CROSSED_BOUNDARY trigger.
        time (OptionalSingleOrList[datetime]): The time parameter for each trigger. Required by
            TIME trigger.
        weapon_name (OptionalSingleOrList[str]): The weapon name parameter for each trigger.
            Optionally required by AGENT_FIRED_WEAPON, WEAPON_FIRED_NEAR_LOCATION, and
            WEAPON_FIRED_NEAR_AGENT triggers.
        agent_affiliation (OptionalSingleOrList[Affiliation]): The affiliation of agents
            that activate the trigger.
            Required by the AGENT_SENSED and AGENTS_SENSED_IN_RANGE trigger.
        confidence_level (OptionalSingleOrList[Confidence]): The confidence level of sense that
            activates the trigger.
            Required by the AGENT_SENSED and AGENTS_SENSED_IN_RANGE trigger.
        agent_types (OptionalSingleOrList[list[AgentType]]): The agent types that
            activate the trigger.
            Required by the AGENT_SENSED and AGENTS_SENSED_IN_RANGE trigger.
        casualty_state (OptionalSingleOrList[AgentCasualtyState]): The casualty state of sense
            that activate the trigger.
            Required by the AGENT_SENSED and AGENTS_SENSED_IN_RANGE trigger.
        sense_range (OptionalSingleOrList[int]): The range of sense that will activate the trigger.
            Required by the AGENT_SENSED and AGENTS_SENSED_IN_RANGE trigger.
    ...
    trigger_config_lists.agent_affiliation_list = create_list(number, agent_affiliation)
    trigger_config_lists.confidence_level_list = create_list(number, confidence_level)
    trigger_config_lists.agent_types_list = create_list(number, agent_types)
    trigger_config_lists.casualty_state_list = create_list(number, casualty_state)
    trigger_config_lists.sense_range_list = create_list(number, sense_range)
...
def create_triggers(
    ...
    agent_affiliation: OptionalSingleOrList[Affiliation] = None,
    confidence_level: OptionalSingleOrList[Confidence] = None,
    agent_types: OptionalSingleOrList[list[AgentType]] = None,
    casualty_state: OptionalSingleOrList[AgentCasualtyState] = None,
    sense_range: OptionalSingleOrList[int] = None,
) -> list[Trigger]:
    ...
    trigger_type_creators: dict[TriggerType, Callable] = {
        ...
        TriggerType.AGENTS_SENSED: _create_agent_sensed_trigger,
        TriggerType.AGENTS_SENSED_IN_RANGE: _create_agent_sensed_in_range_trigger,
    }
