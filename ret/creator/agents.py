...
from ret.behaviours.communicate import CommunicateMissionMessageBehaviour
...
def create_agents(  # flake8: noqa: C501
    ...
    background_orders: OptionalSingleOrList[list[Template[BackgroundOrder]]] = None,
    ...
    pathfinder: OptionalSingleOrList[GridAStarPathfinder] = None,
) -> list[RetAgent]:
    ...
    Args:
        ...
        background_orders (OptionalSingleOrList[list[Template[BackgroundOrder]]]): The initial set of background orders given to each agent. Defaults to None.
    ...
    _background_orders: list[list[Template[BackgroundOrder]]] = create_list_of_lists(number, background_orders)  # type: ignore
    ...
    _pathfinders: list[GridAStarPathfinder] = create_list(number, pathfinder)  # type: ignore
    parameter_lists: dict[str, list[Any]] = {
        ...
        "orders": _orders,
        "background_orders": _background_orders,
        "sensors": _sensors,
    ...
    for a in range(0, number):
        ...
        elif _agent_type[a] is AgentType.ARMOUR:
            invalid_arguments = ["countermeasures", "height_bands"]
            kwargs = update_kwargs(model, parameter_lists, invalid_arguments, a, "ArmourAgent")

            kwargs["behaviours"] = behaviours
            agent = ArmourAgent(**kwargs)

        elif _agent_type[a] is AgentType.MECHANISED_INFANTRY:
            invalid_arguments = ["countermeasures", "height_bands"]
            kwargs = update_kwargs(model, parameter_lists, invalid_arguments, a, "ArmourAgent")

            kwargs["behaviours"] = behaviours
            agent = ArmourAgent(**kwargs)
            agent.agent_type = AgentType.MECHANISED_INFANTRY

        elif _agent_type[a] is AgentType.ARTILLERY:
            invalid_arguments = ["countermeasures", "height_bands"]
            kwargs = update_kwargs(model, parameter_lists, invalid_arguments, a, "ArtilleryAgent")

            kwargs["behaviours"] = behaviours
            agent = ArtilleryAgent(**kwargs)
