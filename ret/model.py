...
from ret.sensing.sensor import ActivelySensingAgentsManager
from ret.visualisation import get_playback_writer_register, JsonWriter
...
if TYPE_CHECKING:
    ...
    from ret.logger.logger import LogConfig
...
class RetModel(Model):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs (Any): Any optional keyword arguments you wish to pass into the Mesa
                Model.
        """
        ...
        if playback_writer is not None:
            ...
            self.playback_writer = JsonWriter(output_folder_name=self.output_path)
    ...
    def check_log_this_timestep(self) -> None:
        """Check if agent position is to be logged this timestep."""
        # Check if current step is multiple of how often to log position.
        if (self.schedule.steps % self.record_position == 0) | (self.schedule.steps == 0):
            self.log_this_step = True
        else:
            self.log_this_step = False

    def log_agent_positions(self):
        """Logs the positions of agents."""
        agents = self.get_all_agents()
        for agent in agents:
            if self.logger.log_position_and_culture:
                self.check_log_this_timestep()
                if self.log_this_step is True:
                    self.logger.log_position_and_culture_record(agent=agent)
        ...
        for agent in agents:
            if isinstance(agent, GroupAgent):
                for subordinate_agent in agent.agents:
                    agents.append(subordinate_agent)  # noqa: B038
