...
ICON_DIR = Path(__file__).parent.joinpath("icons/artillery/")


class DefaultArtilleryWeapon(BasicLongRangedWeapon):
    """Default Weapon for Artillery Agent."""

    def __init__(self):
        """Create a new DefaultArtilleryWeapon."""
        super().__init__(
            name="Default Artillery Weapon",
            radius=3000,
...
class ArtilleryAgent(RetAgent):
    """A class representing a group of Artillery."""

    icon_dict = {
        Affiliation.FRIENDLY: ICON_DIR.joinpath("artilleryagent_friendly.svg"),
        Affiliation.HOSTILE: ICON_DIR.joinpath("artilleryagent_hostile.svg"),
        Affiliation.NEUTRAL: ICON_DIR.joinpath("artilleryagent_neutral.svg"),
        Affiliation.UNKNOWN: ICON_DIR.joinpath("artilleryagent_unknown.svg"),
    }

    killed_icon_dict = {
        Affiliation.FRIENDLY: ICON_DIR.joinpath("killed/artilleryagent_friendly_killed.svg"),
        Affiliation.HOSTILE: ICON_DIR.joinpath("killed/artilleryagent_hostile_killed.svg"),
        Affiliation.NEUTRAL: ICON_DIR.joinpath("killed/artilleryagent_neutral_killed.svg"),
        Affiliation.UNKNOWN: ICON_DIR.joinpath("killed/artilleryagent_unknown_killed.svg"),
    }
    ...
    def __init__(
        ...
        Args:
            ...
            weapons (Optional[list[Weapon]]): List of weapons the agent is armed with. If None,
                a single DefaultArtilleryAgentWeapon will be used. If the caller wishes to create an
                Artillery agent without any weapons, an empty list argument (e.g., `[]` or `list()`)
        ...
        if weapons is None:
            weapons = [DefaultArtilleryWeapon()]
        ...
        super().__init__(
            ...
            agent_type=AgentType.ARTILLERY,
            icon_path=icon_path,
