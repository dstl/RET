...
from ret.weapons.weapon import BasicShortRangedWeapon
...
ICON_DIR = Path(__file__).parent.joinpath("icons/infantry/")


class DefaultInfantryWeapon(BasicShortRangedWeapon):
    """Default Weapon for Infantry Agent."""

    def __init__(self):
        """Create a new DefaultInfantryWeapon."""
        super().__init__(
            name="Default Infantry Weapon",
            radius=3000,
            time_before_first_shot=timedelta(seconds=0),
            time_between_rounds=timedelta(seconds=30),
            kill_probability_per_round=0.5,
            max_angle_inaccuracy=0,
        )


class InfantryAgent(RetAgent):
    """A class representing a group of infantry."""

    icon_dict = {
        Affiliation.FRIENDLY: ICON_DIR.joinpath("infantryagent_friendly.svg"),
        Affiliation.HOSTILE: ICON_DIR.joinpath("infantryagent_hostile.svg"),
        Affiliation.NEUTRAL: ICON_DIR.joinpath("infantryagent_neutral.svg"),
        Affiliation.UNKNOWN: ICON_DIR.joinpath("infantryagent_unknown.svg"),
    }

    killed_icon_dict = {
        Affiliation.FRIENDLY: ICON_DIR.joinpath("killed/infantryagent_friendly_killed.svg"),
        Affiliation.HOSTILE: ICON_DIR.joinpath("killed/infantryagent_hostile_killed.svg"),
        Affiliation.NEUTRAL: ICON_DIR.joinpath("killed/infantryagent_neutral_killed.svg"),
        Affiliation.UNKNOWN: ICON_DIR.joinpath("killed/infantryagent_unknown_killed.svg"),
    }
    ...
    def __init__(
        ...
        Args:
            ...
            weapons (Optional[list[Weapon]]): List of weapons the agent is armed with. If None,
                a single DefaultAirAgentWeapon will be used. If the caller wishes to create an
                infantry agent without any weapons, an empty list argument (e.g., `[]` or `list()`)
