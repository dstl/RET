...
example, Python files for the model and the visualization, anotebook
for analysis, and a Readme with some documentation and discussion).

begin by creating a folder, and either launch anotebook or create a new
...
interpreter), or launch a Jupyternotebook there. Then import the
classes you created. (If you wrote the code in anotebook, obviously
...
session, especially via anotebook, comes in handy: the DataCollector
