...
class FixedBatchRunner:
    """This class is instantiated with a model class.

    And model parameters associated with one or more values.
    It is also instantiated with model and
    ...
    Also note that the parameters_list processing doesn’t use the key value
    pairs, instead it extracts out the values as a list. This can lead to errors
    if the dictionary keys aren’t given in the same order. It also means that
    partial dictionaries cannot be used with a default value for missing
    parameters. As the ‘Parameter product’ and the ‘Latin hypercube sampler’
    both return consistently formatted parameter lists, this won’t cause an
    issue with generated parameter lists, only when manually specified.
        ...
        if count:
            for params in self.parameters_list:
                ...
                all_param_values.append(
                    [self._get_immutable_value(value) for value in params.values()]
                )
    ...
    def _get_immutable_value(self, value):
    ...
    def _prepare_report_table(self, vars_dict, extra_cols=None):
        ...
        if ordered.columns.__contains__("AgentId"):
            ordered.sort_values(by=["Run", "AgentId"], inplace=True)
        else:
...
class BatchRunnerMP(BatchRunner):
    ...
    def _make_model_args_mp(self):
        ...
        Prepare all combinations of parameter values for `run_all`
        Due to multiprocessing requirements of @StaticMethod takes different input,
        hence the similar function.
    ...
    def _run_wrappermp(iter_args):
        ...
        this is primarily to ensure functionality on Windows OS and does not
        impact MAC or Linux distros
        ...
        :param iter_args: List of arguments for model run
            ...
            iter_args[3] = number of time to run model for stochastic/random variation
    ...
    def _result_prep_mp(self, results):
        ...
        Helper Function
        :param results: Takes results dictionary from Processpool and single processor
        debug run and fixes format to
