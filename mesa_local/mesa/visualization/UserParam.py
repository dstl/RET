...
class UserSettableParameter:
    ...
    UserSettableParameter can be used instead of keyword arguments when
    specifying model parameters in an
    instance of a `ModularServer` so that the parameter can be adjusted in
    the UI without restarting the server.

    Validation of correctly-specified params happens on startup of a `ModularServer`.
    Each param is handled
    individually in the UI and sends callback events to the server when an option is updated.
    That option is then
    re-validated, in the `value.setter` property method to ensure input is correct
    from the UI to `reset_model` callback.
    ...
    # Slider input
    slider_option = UserSettableParameter('slider', 'My Slider', value=123,
    min_value=10, max_value=200, step=0.1)
    ...
    def value(self, value):
        ...
        if self.param_type == self.SLIDER:
            ...
            if self._value not in self.choices:
                print(
                    "Selected choice value not in available choices, "
                    "selected first choice from 'choices' list"
                )
