...
def find_background_image_path(canvas_width, canvas_height, canvas_background_file):
    """Establish the path to the background file should one exist.

    The function copies it from wherever specified into the root.
    Also checks the aspect ratio of the input file against the canvas.

    Args:
        canvas_height: ,
        canvas_width: The width and height selected for the canvas.
...
def check_image_aspect_ratio(canvas_width, canvas_height, image_file_path: str):
    ...
    Args:
        canvas_height: ,
        canvas_width: The width and height selected for the canvas.
        image_file_path: Absolute path to the image.
    ...
    if not math.isclose(image_aspect_ratio, space_aspect_ratio, rel_tol=5e-2):
        warnings.warn(
            ...
            + "the image will be stretched to span the space",
            stacklevel=2,
        )
