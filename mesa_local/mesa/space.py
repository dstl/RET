...
class Grid:
    ...
    def __init__(self, width: int, height: int, torus: bool) -> None:
        ...
        self.grid = [[self.default_val() for _ in range(self.height)] for _ in range(self.width)]
    ...
    def __getitem__(
        ...
        if isinstance(index[0], tuple):
            # grid[(x1, y1), (x2, y2)]
            index = cast(Sequence[Coordinate], index)  # noqa: TC006
        ...
        if isinstance(x, int) and isinstance(y, int):
            # grid[x, y]
            index = cast(Coordinate, index)  # noqa: TC006
        ...
        # grid[:, :]
        x, y = (cast(slice, x), cast(slice, y))  # noqa: TC006
        cells = []
...
class SingleGrid(Grid):
    ...
    empties: [Coordinate] = set()

    def __init__(self, width: int, height: int, torus: bool) -> None:
        """Create a new single-item grid.

        Args:
            width: The width and width of the grid
            height: The width and width of the grid
            torus: Boolean whether the grid wraps or not.
...
class ContinuousSpace:
    ...
    def __init__(
        ...
        Args:
            x_max:,
            y_max: Maximum x and y coordinates for the space.
            torus: Boolean for whether the edges loop around.
            x_min:,
            y_min: (default 0) If provided, set the minimum x and y
        ...
        if x_min > x_max:
            raise ValueError(
                f"When creating a space, the minimum x value ({x_min}) cannot be"
                + f"greater than the maximum x value ({x_max})"
            )
        if y_min > y_max:
            raise ValueError(
                f"When creating a space, the minimum y value ({y_min}) cannot be"
                + f"greater than the maximum y value ({y_max})"
            )
