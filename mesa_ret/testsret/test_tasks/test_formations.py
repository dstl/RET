...
from mesa_ret.communication.communicationreceiver import GroupAgentCommunicationReceiver
from mesa_ret.formations import (
    SquareFormationFullColumns,
    SquareFormationFullRows,
    SquareFormationRounded,
)
from mesa_ret.orders.order import GroupAgentOrder, Order
from mesa_ret.orders.tasks.move import GroupFormationMoveTask, MoveTask
from mesa_ret.orders.triggers.immediate import ImmediateTrigger
from mesa_ret.testing.mocks import MockModel2d, MockMoveBehaviour
...
if TYPE_CHECKING:
    ...
    from mesa_ret.formations import Formation
...
def test_basic_move_task():
    ...
    assert type(subordinate_agent_1._orders[0]) == Order
    assert type(subordinate_agent_1._orders[0]._task) == MoveTask
    assert subordinate_agent_1._orders[0]._task._destination == (4900, 4900)  # type: ignore
    assert type(subordinate_agent_1._orders[0]._trigger) == ImmediateTrigger
    ...
    assert type(subordinate_agent_2._orders[0]) == Order
    assert type(subordinate_agent_2._orders[0]._task) == MoveTask
    assert subordinate_agent_2._orders[0]._task._destination == (4900, 5100)  # type: ignore
    assert type(subordinate_agent_2._orders[0]._trigger) == ImmediateTrigger
...
def test_basic_move_include_killed_task():
    ...
    assert type(subordinate_agent_3._orders[0]) == Order
    assert type(subordinate_agent_3._orders[0]._task) == MoveTask
    assert subordinate_agent_3._orders[0]._task._destination == (5100, 4900)  # type: ignore
    assert type(subordinate_agent_3._orders[0]._trigger) == ImmediateTrigger
...
def test_communicate_formation_move_task():
    ...
    assert type(group_agent._orders[0]) == GroupAgentOrder
    assert type(group_agent._orders[0]._task) == GroupFormationMoveTask
    assert type(group_agent._orders[0]._trigger) == ImmediateTrigger
