...
if TYPE_CHECKING:
    ...
    from mesa_ret.visualisation.json_models import RetPlayAgent, RetPlaySpace, RetPlayStepData
...
class RetPlayMapFromPNG(dl.Map):
    ...
    This class currently represents a naieve implementation of the RetPlay map feature. This class
    currently only displays data for a single timestep. Future development is expected to have the
    map iterate through multiple timesteps of data, with marker images and positions updating
    accordingly.
    """
    ...
    def get_default_affiliation_icon(self, agent: RetPlayAgent) -> str:
        ...
        if agent.affiliation == str(Affiliation.FRIENDLY.name):
            ...
            warnings.warn("Unrecognised affiliation type")
            return ""
    ...
    def create_tooltip(agent: RetPlayAgent) -> dl.Tooltip:
        ...
        Args:
            agent (RetPlayAgent): The agent to diaplay data for.
