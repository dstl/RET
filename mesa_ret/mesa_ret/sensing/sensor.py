...
from mesa_ret.sensing.distribution import SingleValueDistribution
from mesa_ret.sensing.perceivedworld import Confidence, PerceivedAgentBasedOnAgent
from mesa_ret.space.culturemap import LineOfSightException
from mesa_ret.space.space import Precipitation
...
if TYPE_CHECKING:
    ...
    from mesa_ret.sensing.distribution import Distribution
...
class SensorFilter(ABC):
    """An abstract class respresenting a filter for sensors.
...
class Sensor(ABC, Template):
    ...
    def run_detection(
        ...
        if self.is_active_sensor & sensor_agent.hiding:
            warnings.warn("Cannot actively sense when a sensor agent is hiding.")
            return
        ...
        if isinstance(sense_direction, tuple):
            ...
            sense_direction_angle = gu._heading_to_clockise_angle_degrees(heading)  # type: ignore
            ...
            if sense_direction in sensor_agent.model.space.areas:
                ...
                sense_direction_angle = gu._heading_to_clockise_angle_degrees(
                    heading  # type: ignore
                )
            else:
                raise (
            ...
            warnings.warn("No sense direction provided, assuming positive x direction.")
    ...
    def _apply_side_effects(
        ...
        Args:
            sensor_agent (RetAgent): The sensin agent.
    ...
    def _get_angle_size_to_target(
        ...
        """Get absolute angle between sensor agent and target agent when looking in the sense direction.
        ...
        """
        angle = (
            gu._heading_to_clockise_angle_degrees(
                (
...
class Acquire1dSensor(Sensor):
    ...
    def __init__(
        ...
        if magnification <= 0:
            raise ValueError(
                f"Acquire 1D Sensor magnification value ({magnification}) must be postive."
            )
