...
from mesa_ret.creator.agents import create_list_of_lists
...
def add_orders_to_agents(
    ...
    if isinstance(agents, Iterable):
        _orders = create_list_of_lists(len(agents), orders)
...
def add_background_orders_to_agents(
    ...
    """
    if isinstance(agents, Iterable):
        _background_orders = create_list_of_lists(len(agents), background_orders)
        for a in range(0, len(agents)):
