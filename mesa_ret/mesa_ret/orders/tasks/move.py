...
from mesa_ret.orders.order import Order, Task
from mesa_ret.orders.triggers.immediate import ImmediateTrigger
from mesa_ret.types import Coordinate3dBand
...
if TYPE_CHECKING:
    ...
    from mesa_ret.types import Coordinate, Coordinate2d, Coordinate2dOr3d, Coordinate3d
...
class FixedDisplacementCalculator(DisplacementCalculator):
    """Displacement calculator for cases where agent always has to move a predefinable distance.
...
class GroupFormationMoveTask(Task):
    ...
    def _do_task_step(self, doer: RetAgent) -> None:
        ...
        if not doer.agents:
            warnings.warn("GroupAgent has no agents to move.")
            return
