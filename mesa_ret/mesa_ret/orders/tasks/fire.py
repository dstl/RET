...
from mesa_ret.orders.tasks.attack import AttackTask
...
if TYPE_CHECKING:
    ...
    from mesa_ret.weapons.weapon import FireSchedule, Weapon
...
class RandomWeaponSelector(WeaponSelector):
    ...
    def select_weapon(self, candidate_weapons: list[Weapon], rand: Random) -> Optional[Weapon]:
        ...
        if len(candidate_weapons) == 0:
            warnings.warn("No weapons are available for firing.")
            return None
...
class FireTask(AttackTask):
    ...
    def _do_task_step(self, doer: RetAgent) -> None:
        ...
        """
        if doer.hiding:
            warnings.warn("Cannot fire while the agent is hiding.")
            return
        ...
        if self.weapon is None:
            warnings.warn("The firer has no suitable weapons for this Task.")
            return
