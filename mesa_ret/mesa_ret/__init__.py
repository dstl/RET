...
__title__ = "ret"
__version__ = "1.0.0"
__license__ = "Apache License, Version 2.0 & DEFCON 705"
__copyright__ = "Copyright %s Dstl" % datetime.date.today().year
...
def copy_map_file(map_location: str, map_destination: str = None) -> None:
    ...
    if path.exists() and path.is_file():
        if pathlib.Path(map_location).suffix != ".png":
            warnings.warn("map file not a PNG filetype.")
            return
        copy_path = os.path.abspath(map_destination)
        os.makedirs(os.path.dirname(copy_path), exist_ok=True)
        shutil.copy(path, copy_path)
    else:
        warnings.warn("map file path either doesn't exist or is not a file.")
