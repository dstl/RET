...
from mesa_ret.behaviours.move import MoveBehaviour, MoveInBandBehaviour
...
from mesa_ret.orders.order import GroupAgentOrder, Order, Task, Trigger
...
if TYPE_CHECKING:
    ...
    from mesa_ret.types import Color, Coordinate, Coordinate2d, Coordinate2dOr3d, Coordinate3d
...
class MockParametrisedModel(MockModel2d):
    ...
    def __init__(self, **kwargs: str) -> None:
        ...
        self.result = np.prod([float(v) for _, v in kwargs.items()])
...
class MockRetPlayStepData(RetPlayStepData):
    ...
    def __init__(self, agent: Optional[RetPlayAgent] = None) -> None:
        ...
        """
        if agent is None:
            super().__init__(step_number=0, agents=[])
        else:
            super().__init__(step_number=0, agents=[agent])
