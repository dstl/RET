...
from mesa_ret.behaviours.behaviourpool import BehaviourHandlers, BehaviourPool, NoEqualityAdder
from mesa_ret.behaviours.sense import SenseSchedule
from mesa_ret.communication.communicationnetwork import CommunicationNetwork
from mesa_ret.orders.background_order import BackgroundOrder
from mesa_ret.orders.order import Order, TaskLogStatus
from mesa_ret.sensing.agentcasualtystate import AgentCasualtyState
from mesa_ret.sensing.perceivedworld import PerceivedWorld
...
if TYPE_CHECKING:
    ...
    from mesa_ret.space.pathfinder import GridAStarPathfinder
    from mesa_ret.template import Template
    from mesa_ret.types import Coordinate, Coordinate2dOr3d
...
class RetAgent(Agent):
    ...
    def kill(self, shot_id: Optional[int] = None, killer: Optional[RetAgent] = None) -> None:
        """Kill the agent and update icon.

        Args:
            shot_id (str): Unique ID for the shot
            killer (RetAgent, optional): The agent who performed the kill
        """
        self._killed = True
        for c in (c for c in self._countermeasures if c.deployed and not c.persist_beyond_deployer):
            c.kill(self.model.get_time())
        self.set_icon()
        self.model.logger.log_death(target=self, killer=killer, shot_id=shot_id)
